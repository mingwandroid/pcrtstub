...
void
printHeader (FILE * fp, char *commentBegin, char *commentEnd)
{
...
static void
printStubCode (FILE * fp, const char *hname, const char *sym)
{
  printHeader (fp, "/*", " */");
...
static FILE *
addSrcFopen (FILE * makefile, const char *pth, const char *filename,
	     int isFinalFile, const char *mode)
{
...
static void
addSrcEmulated (FILE * makefile, const char *filename, int isFinalFile)
{
  /* VPATH is used for the directory, so get filename part */
  makefileAddSourceFile (makefile, filename, isFinalFile);
}


void
outputSyms (char const *vpath, char const *emul_sources)
{
  FILE *fp;
  FILE *makefile;
  sSymbol *l = NULL, *l_last_emulated = NULL, *l_last_nonemulated = NULL;
  ...
  makefile = makefileCreate (pth, vpath);
  ...
  while (l != NULL)
	...
	{
	  fp =
	    addSrcFopen (makefile, pth, unifyCat ("stub_", hname), 0, "wb");
  ...
  if (emul_sources != NULL && emul_sources[0] != '\0')
    {
      makefileStartEmulatedGroups (makefile);
      const char *next_source = emul_sources;
      const char *this_source = 0;
      while (*next_source != '\0')
	{
	  char srcfilename[1024];
	  this_source = next_source;
	  const char *colon = strchr (this_source, ':');
	  next_source =
	    (colon == NULL) ? this_source + strlen (this_source) : colon;
	  memcpy (srcfilename, this_source, next_source - this_source);
	  addSrcEmulated (makefile, srcfilename,
			  (l == l_last_emulated) ? 1 : 0);
	}
    }
